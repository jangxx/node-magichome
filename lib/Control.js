...
class Control {
	 ...
	 * @param {boolean} options.wait_for_reply Wait for the controllers to send a byte as acknowledgement (Default: true)
	...
	_sendColorChangeCommand(red, green, blue, ww, mask, callback) {
		red = clamp(red, 0, 255);
		green = clamp(green, 0, 255);
		blue = clamp(blue, 0, 255);
		ww = clamp(ww, 0, 255);

		let cmd_buf = Buffer.from([ 0x31, red, green, blue, ww, mask, 0x0f ]);
	 ...
	 */
  	setColorAndWarmWhite(red, green, blue, ww, callback) {
		return this._sendColorChangeCommand(red, green, blue, ww, 0, callback);
	}
	...
	setColor(red, green, blue, callback) {
		if (this._options.apply_masks) {
			return this._sendColorChangeCommand(red, green, blue, 0, 0xF0, callback);
		} else {
	 ...
	 */
  	setWarmWhite(ww, callback) {
		if (this._options.apply_masks) {
			return this._sendColorChangeCommand(0, 0, 0, ww, 0x0F, callback);
		} else {
	...
	queryState(callback) {
		...
		let promise = new Promise((resolve, reject) => {
			...
			switch (state.type) {
				case 0x25: // possible other device types?
