...
class Control {
	...
	setPower(on, callback) {
		...
		let promise = new Promise((resolve, reject) => {
			...
			if (data.length == 0 && !this._options.wait_for_reply) return true;
	 ...
	 */
  	setColorAndWarmWhite(red, green, blue, ww, callback) {
		...
		let promise = new Promise((resolve, reject) => {
			...
			if (data.length == 0) return !this._options.wait_for_reply;
	...
	startEffectMode(callback) {
		let promise = new Promise(resolve => {
			new EffectInterface(this._address, PORT, this._options, (effect_interface) => {
