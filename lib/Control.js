...
class Control {
  ...
  setWarmWhite(level, callback) {
		var ww_value = (this._characteristics.ww_min_0) ? 0 : 0xFF;
    var percentage = (level * 255) / 100;

		var cmd_buf = Buffer.from([0x31, 0, 0, 0, percentage, this._characteristics.set_color_magic_bytes[0], this._characteristics.set_color_magic_bytes[1]]);
  ...
  setColorAndWarmWhite(red, green, blue, level, callback) {
    ...
    red = clamp(red, lower_bound, 255);
		...
		var cmd_buf = Buffer.from([0x31, red, green, blue, percentage, this._characteristics.set_color_magic_bytes[0], this._characteristics.set_color_magic_bytes[1]]);
