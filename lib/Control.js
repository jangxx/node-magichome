...
function determineMode(resp) {
	const patternCodes = [resp.readUInt8(3), resp.readUInt8(4)];

	if(resp.readUInt8(1) === 161) {
		if(patternCodes[0] === 0 && patternCodes[1] === 97 ) {
			return "color";
		} else if (isIaPatternActive(resp)) {
			return "ia_pattern";
		}
	}else {
		if(patternCodes[0] === 0x61) {
			return "color";
		} else if (patternCodes[0] === 0x62) {
			return "special";
		} else if (patternCodes[0] === 0x60) {
			return "custom";
		} else if (patternCodes[0] >= 0x25 && patternCodes[0] <= 0x38) {
			return "pattern";
		}
	}
...
function isIaPatternActive(resp) {
	return resp.readUInt8(3) === 0 && resp.readUInt8(4) >= 100 || resp.readUInt8(3) === 1 && resp.readUInt8(4) <= 143;
}

function determinePattern(resp) {
	const functionCodes = [resp.readUInt8(3), resp.readUInt8(4)];

	if (functionCodes[0] >= 0x25 && functionCodes[0] <= 0x38) {
		for(let pattern_name in patterns) {
			if(patterns[pattern_name] === functionCodes[0] ) return pattern_name;
		}
	}

	if(isIaPatternActive(resp)) {
		return functionCodes[0] === 1 ? functionCodes[1]+256-99 : functionCodes[1]-99;
	}
}

function delayToSpeed(delay, resp) {
	if(isIaPatternActive(resp)) {
		return delay;
	}else{
...
class Control {
	...
	setIaPattern(code, speed, callback) {
		if(code < 1 || code > 300) {
			let promise = Promise.reject(new Error("Invalid code"));
		...
		const functionNumber = code + 99

		const bufferArray = [0x61];
		bufferArray.push(functionNumber >= 255 ? 1 : 0);
		bufferArray.push(functionNumber >= 255 ? functionNumber-256 : functionNumber);
	...
	queryState(callback) {
		...
		let promise = new Promise((resolve, reject) => {
			...
			let state = {
				...
				speed: delayToSpeed(data.readUInt8(5), data),
				color: {
