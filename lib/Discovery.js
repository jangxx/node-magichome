	...
	scan(timeout = 500, callback = undefined) {
		let promise = new Promise((resolve, reject) => {
			...
			if (os.platform() == "win32") {
				...
				for(let i in ifaces) {
					adresses = adresses.concat(ifaces[i].filter(iface => iface.family == "IPv4" && !iface.internal).map(iface => iface.address));
				}
			} else {
				adresses = [ BROADCAST_ADDR ];
			}
			...
			socket.on('listening', () => {
				...
				adresses.forEach(addr => socket.send('HF-A11ASSISTHREAD', DISCOVERY_PORT, addr));
			});
