	...
	scan(timeout = 500, callback = undefined) {
		let promise = new Promise((resolve, reject) => {
			let socket = dgram.createSocket('udp4'),
				address = (process.platform == 'win32' || process.platform == 'win64') ? BROADCAST_ADDRESSES['win'] : BROADCAST_ADDRESSES['other'];
			...
			socket.on('listening', () => {
				...
				socket.send('HF-A11ASSISTHREAD', DISCOVERY_PORT, address);
			});
