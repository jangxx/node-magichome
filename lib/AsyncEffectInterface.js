...
class AsyncEffectInterface {
    ...
    _sendCommand(buf) {
		...
		if(this._color_ack) {
			...
			this._socket.write(command, "utf8", () => {
                ...
                }, 100); // 100 ms minimum delay
			});
		}
	}

    _sendColorChangeCommand(red, green, blue, ww, cw, mask) {
		const cmd_buf = this._parent._buildColorChangeCommand(red, green, blue, ww, cw, mask);
    ...
    hasPinnedTime() {
        return this._pinnedTime !== null;
    }

    pinTime() {
        this._pinnedTime = new Date().getTime();
    }

    async delayRemaining(milliseconds) {
        const now = new Date().getTime();
        const remaining = this._pinnedTime + milliseconds - now;

        if (remaining > 0) {
            await this.delay(remaining);
        }

        this._pinnedTime = this._pinnedTime + milliseconds;
    }
...
module.exports = { AsyncEffectInterface };